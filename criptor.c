...
void criptare(const char *imagine_primita, const char *imagine_returnata, const char *cheie)
{
    pixel *imagine=NULL;
    ...
    if(secventa==NULL)
    {
        printf("alocare nereusita secventa aleatoare criptare");
        return;
    }

    int *per=genereaza_permutare_durstenfeld(secventa,dim);


    permuta(per,imagine,dim);



    xorare_criptare(imagine,secventa,dim,sv);
...
int main(int argc, char const *argv[])
{
    ...
    if (argc == 3)
    {    
        criptare(argv[1],argv[2],cheie);
        frecv_test(argv[2]);
    }
    else if(argc == 2)
    {
        char ciphered_file[strlen(argv[1])+10];
        strcpy(ciphered_file,"ciphered_");
        strcat(ciphered_file,argv[1]);
         
        criptare(argv[1],ciphered_file,cheie);
        frecv_test(ciphered_file);
    }
